...
def generateRandomLine(isVertical, lowerBound = 0, upperBound = 8, gridSize = 10):
    length = random.randint(lowerBound, upperBound)/gridSize
    originX1 = random.randint(lowerBound, gridSize)/gridSize
    originY1 = random.randint(lowerBound, gridSize)/gridSize
    originX2 = 0
    originY2 = 0
    if isVertical:
        originX2 = originX1
...
def isWithinBounds(line, isVertical, gridSize = 10):
    if isVertical:
        return line.x0 >= 1/gridSize and line.x1 <= 9/gridSize
    
    else:
        return line.y0 >= 1/gridSize and line.y1 <= 9/gridSize


def chooseNextState(previousState, table=[[.8, .2], [.3, .7]]):
    randomRoll = random.random()
    if randomRoll > 1 - table[previousState][0]:
        # A!
        return 0
    else:
        # B!
...
def chooseNextLine(previousState, previousLine, gridSize= 10):
    nextState = chooseNextState(previousState)
    nextLine = Line(0, 0, 0, 0)
    if nextState == 0:
        nextLine = generateRandomLine(chooseOrientation())
    else:
        # Intersect: given line's corrdinates, choose point along line to draw through with prependicular line
        if previousLine.x0 == previousLine.x1:
            
            nextLine = generateRandomLine(False, math.floor(previousLine.y0*gridSize), math.floor(previousLine.y1*gridSize), gridSize)

        else:
            nextLine = generateRandomLine(True, math.floor(previousLine.x0*gridSize), math.floor(previousLine.x1*gridSize), gridSize)

    return [nextLine, nextState]
...
if __name__ == "__main__":
    ...
    goodLines = 0
    previousState = 0
    previousLine = generateRandomLine(chooseOrientation())

    while goodLines <= 12:
        [newLine, newState] = chooseNextLine(previousState, previousLine, 20)
